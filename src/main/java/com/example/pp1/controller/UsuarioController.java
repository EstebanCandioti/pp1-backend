...
import com.example.pp1.DTO.LoginResponse;
...
import com.example.pp1.security.JwtUtil;
...
public class UsuarioController {
    ...
    @PostMapping
    public ResponseEntity<?> actualizarUsuario(@Valid @RequestBody ActualizarUsuarioDTO usuario,
            BindingResult resultado) {
        if (resultado.hasErrors()) {
            String errores = resultado.getFieldErrors().stream().map(FieldError::getDefaultMessage)
    ...
    public ResponseEntity<String> registrarUsuario(@Valid @RequestBody RegistrarUsuarioDTO usuario,
            BindingResult resultado) {
        if (resultado.hasErrors()) {
            /*
             * GetFieldErrors devuelve un array con los campos que tienen errores, comienza
             * un stream para mapear los datos y los une en una sola respuesta
             */
            String errores = resultado.getFieldErrors().stream().map(FieldError::getDefaultMessage)
                    .collect(Collectors.joining(" , "));
            return ResponseEntity.badRequest().body(errores);
    ...
    public ResponseEntity<?> login(@RequestBody Login body) {
        ...
        if (respuesta == UsuarioService.resultadoPeticiones.logeado) {
            ...
            // Devolver token + usuario (sin password por @JsonIgnore)
